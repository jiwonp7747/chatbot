...
async def generate_response_node(
    ...
    # RAG 검색 결과가 있으면 컨텍스트에 포함
    rag_results = state.get("rag_results", [])
    if rag_results:
        rag_context = _format_rag_results(rag_results)
        messages.append({
            "role": "system",
            "content": f"다음은 관련 문서 검색 결과입니다. 사용자의 질문에 답변할 때 이 정보를 참고하세요:\n\n{rag_context}"
        })
        logger.info(f"📖 RAG 검색 결과 {len(rag_results)}개 포함")
...
def _format_rag_results(rag_results: list) -> str:
    """
    RAG 검색 결과를 사람이 읽을 수 있는 형식으로 포맷팅
    ...
    for idx, result in enumerate(rag_results, 1):
        ...
        formatted.append(f"[문서 {idx}] (유사도: {similarity:.2f}, 유형: {memory_type})")
        if tags:
            formatted.append(f"  태그: {', '.join(tags[:5])}")
